...
std::vector<Point> find_accessible(const std::vector<std::vector<bool>>& grid) {
    ...
    for (int row_idx = 0; row_idx < grid.size(); ++row_idx) {
        for (int col_idx = 0; col_idx < grid[row_idx].size(); ++col_idx) {
            ...
            if (is_accessible(grid, row_idx, col_idx)) {
                accessible.push_back({row_idx, col_idx});
            }
        }
    }
    return accessible;
...
int main(int argc, char* argv[]) {
    ...
    int removed = 0;
    while (true) {
        std::vector<Point> accessible = find_accessible(grid);
        if (accessible.empty()) {
            break;
        }
        for (const Point& point : accessible) {
